...
flag = 0;
cnt=1;
sizeOfPointsArray = ceil(sqrt((imageHeight * imageHeight) + (imageWidth * imageWidth)));
whitePixels = length(edgeImage(edgeImage==1));
%set the image to all -1
SWTImage = zeros(imageHeight,imageWidth)-1;
pointStruct = struct('x',0,'y',0);
rayStruct = struct('p',0,'q',0,'points',pointStruct);
raysList = repmat(rayStruct, whitePixels);
